...
import shutil
import gnupg
from setup import envFile, file_mapping_franchise_code_map
...
inbound_spark_management = {
    ...
    "table_name": "franchise_code_map",  # The name of the table being affected
...
def check_and_download_file(sftp):
    ...
    try:
        ...
        for region, filename_prefix in file_mapping_franchise_code_map.items():
                    ...
                    if file.startswith(filename_prefix) and current_date in file:
                        ...
                        try:
                            ...
                            if output_csv_path: 
                                ...
                                franchise_code_map_table_step_2_insert_into_main(spark,properties,db_url)
                                Franchise_Code_Map_table_step_3_update_into_main()
...
def data_to_insert_staging_table(csv, hash_table_db, url, properties):
    try:
        ...
        csv_to_insert_stage.write.jdbc(url=url, table="franchise_code_map_insert_staging", mode="append", properties=properties)
        ...
        missing_code_to_hash_table.write.jdbc(url=url, table="franchise_code_map_hash", mode="append", properties=properties)
...
def data_to_update_staging_table(csv, hash_table_db, url, properties):
    try:
        ...
        csv_to_update_stage.write.jdbc(url=url, table="franchise_code_map_update_staging", mode="append", properties=properties)    
        ...
        start_hash_time = time.time()
        mismatch_hash_to_hash_temp.write.jdbc(url=url, table="franchise_code_map_hash_staging", mode="append", properties=properties)    
        log_time(start_time=start_hash_time, str="csv hash to hash stage")
    except Exception as e:
...
def compare_and_insert(df, main_table_df):
    try:
        ...
        if mismatch_df.count() > 0:
            ...
            mismatch_df.write.jdbc(url=db_url, table="franchise_code_map", mode="append")
    except Exception as e:
...
def main(local_file_path,spark,properties,url):
    ...
    try:
       ...
       # Assuming 'csv' is your DataFrame
        ...
        hash_table_db = spark.read.jdbc(url=url, table="franchise_code_map_hash", properties=properties)
...
def franchise_code_map_table_step_2_insert_into_main(spark, properties, db_url):
    connection = None
    try:
        # psycopg2 Property
        logger.info("Step 2 Started . method NAme franchise_code_map_table_step_2_insert_into_main")
        ...
        sourceDF = spark.read.jdbc(db_url, table="franchise_code_map_insert_staging", properties=properties)
        ...
        sourceDF.select(columns_to_insert).write.jdbc(db_url, table="franchise_code_map", mode="append", properties=properties)
        
        # Insert data count into the logging dictionary
        inbound_spark_management["data_insert_count"] = sourceDF.count()
        
        # Truncate staging table
        cursor.execute("TRUNCATE TABLE franchise_code_map_insert_staging")
        connection.commit()
        ...
        # Log success message
        inbound_spark_management["status"] = "Success"
        inbound_spark_management["success_message"] = "franchise_code_map_table_step_2_insert_into_main executed successfully"
    except Exception as e:
        logger.error(f"Error in franchise_code_map_table_step_2_insert_into_main: {e}")
...
def Franchise_Code_Map_table_step_3_update_into_main():
    connection = None
    try:
        logger.info("----------------Step 3 Started Method Name Franchise_Code_Map_table_step_3_update_into_main ")
        # psycopg2 Property
        connection = psycopg2.connect(user=db_user, password=db_password, host=db_host, port=db_port, database=db_name)
        cursor = connection.cursor()
        ...
        sql_update_query  = """
                BEGIN;
                UPDATE Franchise_Code_Map
                SET
                    name = sourceTable.name,
                    fdscd = sourceTable.fdscd,
                ...
                UPDATE Franchise_Code_Map_hash  
                SET
                    hash_value = temp.hash_value
                FROM
                    Franchise_Code_Map_hash_staging temp
                WHERE
                    Franchise_Code_Map_hash.code = temp.code;

                TRUNCATE TABLE Franchise_Code_Map_hash_staging;
        ...
        logger.error(f"Error in Franchise_Code_Map_table_step_3_update_into_main: {e}")
        inbound_spark_management["status"] = "Failure"
        inbound_spark_management["error_message"] = f'Error in Franchise_Code_Map_table_step_3_update_into_main: {e}'
    finally:
        if connection:
            cursor.close()
            connection.close()
