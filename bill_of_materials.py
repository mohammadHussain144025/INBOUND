...
import shutil
import gnupg
from setup import envFile, file_mapping_bill_of_materials
...
inbound_spark_management = {
    ...
    "table_name": "bill_of_materials",  # The name of the table being affected
...
schema_dict = {
    ...
    "comst": StringType(),
    "comqty": IntegerType(),
    "crtdat": TimestampType(),
    "revdat": TimestampType(),
    "revnum": StringType(),
...
def simplify_colum_with_table(df):
    try:
        ...
        df = df.filter((col('assemb').isNotNull()) & 
                       (col('compnt').isNotNull()))
        df = df.dropDuplicates(['assemb', 'compnt'])
        
        logger.info("Rows with null values in 'assemb', 'compnt' dropped "
                    "and duplicates based on 'assemb', 'compnt' removed")
        return df
...
def check_and_download_file(sftp):
    ...
    try:
        ...
        for region, filename_prefix in file_mapping_bill_of_materials.items():
                    ...
                    if file.startswith(filename_prefix) and current_date in file:
                        ...
                        try:
                            ...
                            if output_csv_path:
                                ...
                                bill_of_materials_table_step_2_insert_into_main(spark,properties,db_url)
                                bill_of_materials_table_step_3_update_into_main()
...
def read_csv_data(path_to_csv, spark):
    try:
        ...
        # Add 'new_code' column assemb', 'compnt'
        df = df.withColumn('code', concat_ws("",col('assemb'), col('compnt')))
        return df
    except Exception as e:
...
def data_to_insert_staging_table(csv, hash_table_db, url, properties):
    try:
        ...
        csv_to_insert_stage.write.jdbc(url=url, table="bill_of_materials_insert_staging", mode="append", properties=properties)
        ...
        missing_code_to_hash_table.write.jdbc(url=url, table="bill_of_materials_hash", mode="append", properties=properties)
...
def data_to_update_staging_table(csv, hash_table_db, url, properties):
    try:
        ...
        csv_to_update_stage.write.jdbc(url=url, table="bill_of_materials_update_staging", mode="append", properties=properties)    
        ...
        start_hash_time = time.time()
        mismatch_hash_to_hash_temp.write.jdbc(url=url, table="bill_of_materials_hash_staging", mode="append", properties=properties)    
        log_time(start_time=start_hash_time, str="csv hash to hash stage")
    except Exception as e:
...
def compare_and_insert(df, main_table_df):
    try:
        ...
        if mismatch_df.count() > 0:
            ...
            mismatch_df.write.jdbc(url=db_url, table="bill_of_materials", mode="append")
    except Exception as e:
...
def main(local_file_path,spark,properties,url):
    ...
    try:
       ...
       # Assuming 'csv' is your DataFrame
        ...
        hash_table_db = spark.read.jdbc(url=url, table="bill_of_materials_hash", properties=properties)
...
def bill_of_materials_table_step_2_insert_into_main(spark, properties, db_url):
    connection = None
    try:
        # psycopg2 Property
        logger.info("Step 2 Started . method NAme bill_of_materials_table_step_2_insert_into_main")
        ...
        sourceDF = spark.read.jdbc(db_url, table="bill_of_materials_insert_staging", properties=properties)
        ...
        sourceDF.select(columns_to_insert).write.jdbc(db_url, table="bill_of_materials", mode="append", properties=properties)
        
        # Insert data count into the logging dictionary
        inbound_spark_management["data_insert_count"] = sourceDF.count()
        
        # Truncate staging table
        cursor.execute("TRUNCATE TABLE bill_of_materials_insert_staging")
        connection.commit()
        ...
        # Log success message
        inbound_spark_management["status"] = "Success"
        inbound_spark_management["success_message"] = "bill_of_materials_table_step_2_insert_into_main executed successfully"
    except Exception as e:
        logger.error(f"Error in bill_of_materials_table_step_2_insert_into_main: {e}")
...
def bill_of_materials_table_step_3_update_into_main():
    connection = None
    try:
        logger.info("----------------Step 3 Started Method Name bill_of_materials_table_step_3_update_into_main ")
        # psycopg2 Property
        connection = psycopg2.connect(user=db_user, password=db_password, host=db_host, port=db_port, database=db_name)
        cursor = connection.cursor()
        ...
        sql_update_query  = """
            ...
            SET
                ...
                assemb = sourceTable.assemb,
                lineno = sourceTable.lineno,
                compnt = sourceTable.compnt,
                comst = sourceTable.comst,
                comqty = sourceTable.comqty,
                crtdat = sourceTable.crtdat,
                revdat = sourceTable.revdat,
                revnum = sourceTable.revnum,
            ...
            FROM 
                bill_of_materials_update_staging AS sourceTable
            WHERE
                bill_of_materials.assemb = sourceTable.assemb AND
                bill_of_materials.compnt = sourceTable.compnt;
                

            TRUNCATE TABLE bill_of_materials_update_staging;

            UPDATE bill_of_materials_hash  
            SET
                hash_value = temp.hash_value
            FROM
                bill_of_materials_hash_staging temp
            WHERE
                bill_of_materials_hash.code = temp.code;

            TRUNCATE TABLE bill_of_materials_hash_staging;
        ...
        logger.error(f"Error in bill_of_materials_table_step_3_update_into_main: {e}")
        inbound_spark_management["status"] = "Failure"
        inbound_spark_management["error_message"] = f'Error in bill_of_materials_table_step_3_update_into_main: {e}'
    finally:
        if connection:
            cursor.close()
            connection.close()
