...
import shutil
import gnupg
from setup import envFile, file_mapping_alternate_part
...
inbound_spark_management = {
    ...
    "table_name": "alternate_parts",  # The name of the table being affected
...
schema_dict = {
    "name": StringType(),
    "code": IntegerType(),
    "root_part": StringType(),
    "xref_part": StringType(),
    "diff_flg": StringType(),
    "diff_str": StringType(),
    "brandsort": StringType(),
    "rgcd": StringType(),
...
def simplify_colum_with_table(df):
    try:
        ...
        df = df.filter((col('root_part').isNotNull()) & 
                       (col('xref_part').isNotNull()))
        df = df.dropDuplicates(['root_part', 'xref_part'])
        
        logger.info("Rows with null values in 'root_part', 'xref_part' dropped "
                    "and duplicates based on 'root_part', 'xref_part' removed")
        return df
...
def check_and_download_file(sftp):
    ...
    try:
        ...
        for region, filename_prefix in file_mapping_alternate_part.items():
...
def read_csv_data(path_to_csv, spark):
    try:
        ...
        # Add 'new_code' column root_part', 'xref_part'
        df = df.withColumn('code', concat_ws("",col('root_part'), col('xref_part')))
        return df
    except Exception as e:
...
def data_to_insert_staging_table(csv, hash_table_db, url, properties):
    try:
        ...
        csv_to_insert_stage.write.jdbc(url=url, table="alternate_parts_insert_staging", mode="append", properties=properties)
        ...
        missing_code_to_hash_table.write.jdbc(url=url, table="alternate_parts_hash", mode="append", properties=properties)
...
def data_to_update_staging_table(csv, hash_table_db, url, properties):
    try:
        ...
        csv_to_update_stage.write.jdbc(url=url, table="alternate_parts_update_staging", mode="append", properties=properties)    
        ...
        start_hash_time = time.time()
        mismatch_hash_to_hash_temp.write.jdbc(url=url, table="alternate_parts_hash_staging", mode="append", properties=properties)    
        log_time(start_time=start_hash_time, str="csv hash to hash stage")
    except Exception as e:
...
def compare_and_insert(df, main_table_df):
    try:
        ...
        if mismatch_df.count() > 0:
            ...
            mismatch_df.write.jdbc(url=db_url, table="alternate_parts", mode="append")
    except Exception as e:
...
def main(local_file_path,spark,properties,url):
    ...
    try:
       ...
       # Assuming 'csv' is your DataFrame
        ...
        hash_table_db = spark.read.jdbc(url=url, table="alternate_parts_hash", properties=properties)
...
def alternate_parts_table_step_2_insert_into_main(spark, properties, db_url):
    connection = None
    try:
        # psycopg2 Property
        logger.info("Step 2 Started . method Name alternate_parts_table_step_2_insert_into_main")
        ...
        sourceDF = spark.read.jdbc(db_url, table="alternate_parts_insert_staging", properties=properties)
        ...
        sourceDF.select(columns_to_insert).write.jdbc(db_url, table="alternate_parts", mode="append", properties=properties)
        
        # Insert data count into the logging dictionary
        inbound_spark_management["data_insert_count"] = sourceDF.count()
        
        # Truncate staging table
        cursor.execute("TRUNCATE TABLE alternate_parts_insert_staging")
        connection.commit()
        ...
        # Log success message
        inbound_spark_management["status"] = "Success"
        inbound_spark_management["success_message"] = "alternate_parts_table_step_2_insert_into_main executed successfully"
    except Exception as e:
        logger.error(f"Error in alternate_parts_table_step_2_insert_into_main: {e}")
...
def alternate_parts_table_step_3_update_into_main():
    connection = None
    try:
        logger.info("----------------Step 3 Started Method Name alternate_parts_table_step_3_update_into_main ")
        # psycopg2 Property
        connection = psycopg2.connect(user=db_user, password=db_password, host=db_host, port=db_port, database=db_name)
        cursor = connection.cursor()
        ...
        sql_update_query  = """
            BEGIN;
            UPDATE alternate_parts
            SET
                name = sourceTable.name,
                root_part = sourceTable.root_part,
                xref_part = sourceTable.xref_part,
                diff_flg = sourceTable.diff_flg,
                diff_str = sourceTable.diff_str,
                brandsort = sourceTable.brandsort,
            ...
            FROM 
                alternate_parts_update_staging AS sourceTable
            WHERE
                alternate_parts.root_part = sourceTable.root_part AND
                alternate_parts.xref_part = sourceTable.xref_part;

            TRUNCATE TABLE alternate_parts_update_staging;

            UPDATE alternate_parts_hash  
            SET
                hash_value = temp.hash_value
            FROM
                alternate_parts_hash_staging temp
            WHERE
                alternate_parts_hash.code = temp.code;

            TRUNCATE TABLE alternate_parts_hash_staging;
        ...
        logger.error(f"Error in alternate_parts_table_step_3_update_into_main: {e}")
        inbound_spark_management["status"] = "Failure"
        inbound_spark_management["error_message"] = f'Error in alternate_parts_table_step_3_update_into_main: {e}'
    finally:
        if connection:
            cursor.close()
            connection.close()
