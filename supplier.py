...
inbound_spark_management = {
    ...
    "table_name": "supplier",  # The name of the table being affected
...
def simplify_colum_with_table(df):
    try:
        # Drop rows where any of 'vacode', 'rgcd' is null 
        df = df.filter((col('vacode').isNotNull()) & 
                       (col('rgcd').isNotNull()))
                       
        
        # Drop duplicates based on 'rgcd', 'xrfptno', 'xricno'
        df = df.dropDuplicates(['vacode', 'rgcd'])
        
        logger.info("Rows with null values in 'vacode', 'rgcd' dropped "
                    "and duplicates based on 'vacode', 'rgcd' removed")
        return df
...
def read_csv_data(path_to_csv, spark):
    try:
        ...
        # Add 'new_code' column rgcd', 'vacode'
        df = df.withColumn('code', concat_ws("",col('vacode'), col('rgcd')))
        return df
    except Exception as e:
...
def data_to_insert_staging_table(csv, hash_table_db, url, properties):
    try:
        ...
        csv_to_insert_stage = csv_to_insert_stage.drop('code', 'record_action')
        csv_to_insert_stage.write.jdbc(url=url, table="supplier_insert_staging", mode="append", properties=properties)
        ...
        missing_code_to_hash_table.write.jdbc(url=url, table="supplier_hash", mode="append", properties=properties)
...
def data_to_update_staging_table(csv, hash_table_db, url, properties):
    try:
        ...
        start_time = time.time()
        csv_to_update_stage = csv_to_update_stage.drop('code', 'record_action')
        csv_to_update_stage.write.jdbc(url=url, table="supplier_update_staging", mode="append", properties=properties)    
        ...
        start_hash_time = time.time()
        mismatch_hash_to_hash_temp.write.jdbc(url=url, table="supplier_hash_staging", mode="append", properties=properties)    
        log_time(start_time=start_hash_time, str="csv hash to hash stage")
    except Exception as e:
...
def main(local_file_path,spark,properties,url):
    ...
    try:
       ...
       # Assuming 'csv' is your DataFrame
        ...
        hash_table_db = spark.read.jdbc(url=url, table="supplier_hash", properties=properties)
...
def supplier_table_step_2_insert_into_main(spark, properties, db_url):
    connection = None
    logger.info("Step 2 Started. Method Name: supplier_table_step_2_insert_into_main")
    ...
    try:
        ...
        start_time = time.time()
        sourceDF = spark.read.jdbc(db_url, table="supplier_insert_staging", properties=properties)
        ...
        sourceDF.select(columns_to_insert).write.jdbc(db_url, table="supplier", mode="append", properties=properties)
        inbound_spark_management["data_insert_count"] = source_count
        
        # Truncate staging table
        cursor.execute("TRUNCATE TABLE supplier_insert_staging")
        connection.commit()
        logger.info("Staging table truncated successfully.")
        
        # Log success message
        inbound_spark_management["status"] = "Success"
        inbound_spark_management["success_message"] = "supplier_table_step_2_insert_into_main executed successfully"
        
    except Exception as e:
        logger.error(f"Error in supplier_table_step_2_insert_into_main: {e}")
...
def supplier_table_step_2_update_into_main():
    connection = None
    try:
        logger.info("----------------Step 3 Started Method Name supplier_table_step_2_update_into_main ")
        # psycopg2 Property
        connection = psycopg2.connect(user=db_user, password=db_password, host=db_host, port=db_port, database=db_name)
        cursor = connection.cursor()
        ...
        sql_update_query  = """
            ...
            SET
                ...
                vastat = sourceTable.vastat,
                vacode = sourceTable.vacode,
                vaname = sourceTable.vaname,
                vaadr1 = sourceTable.vaadr1,
                vaadr2 = sourceTable.vaadr2,
                vcity = sourceTable.vcity,
                vstate = sourceTable.vstate,
                vzip = sourceTable.vzip,
                vcntct = sourceTable.vcntct,
                vaactr = sourceTable.vaactr ,
                vphone = sourceTable.vphone,
                vreltp = sourceTable.vreltp,
                vndtyp = sourceTable.vndtyp,
            ...
            FROM 
                supplier_update_staging AS sourceTable
            WHERE
                supplier.vacode = sourceTable.vacode AND
                supplier.rgcd = sourceTable.rgcd;

            TRUNCATE TABLE supplier_update_staging;

            UPDATE supplier_hash  
            SET
                hash_value = temp.hash_value
            FROM
                supplier_hash_staging temp
            WHERE
                supplier_hash.code = temp.code;

            TRUNCATE TABLE supplier_hash_staging;
        ...
        logger.error(f"Error in supplier_table_step_2_update_into_main: {e}")
        inbound_spark_management["status"] = "Failure"
        inbound_spark_management["error_message"] = f'Error in supplier_table_step_2_update_into_main: {e}'
    finally:
        if connection:
            cursor.close()
            connection.close()
