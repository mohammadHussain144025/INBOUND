...
import shutil
import gnupg
from setup import envFile, file_mapping_franchise
...
inbound_spark_management = {
    ...
    "table_name": "franchise",  # The name of the table being affected
...
schema_dict = {
    ...
    "frands": StringType(),
    "francdn": IntegerType(),
    "frprotect": StringType(),
    "frovr": StringType(),
    "froemflag": StringType(),
    "frdstflag": StringType(),
    "freucflag": StringType(),
    "frmerpbook": StringType(),
    "freucpbook":StringType(),
    "fransmsg":StringType(),
    "paexcept":IntegerType(),
...
def simplify_colum_with_table(df):
    try:
        ...
        df = df.filter(col('francdn').isNotNull())
        df = df.dropDuplicates(['francdn'])
        return df
...
def check_and_download_file(sftp):
    ...
    try:
        ...
        for region, filename_prefix in file_mapping_franchise.items():
                    ...
                    if file.startswith(filename_prefix) and current_date in file:
                        ...
                        try:
                            ...
                            if output_csv_path:  
                                ...
                                franchise_table_step_2_insert_into_main(spark,properties,db_url)
                                franchise_table_step_3_update_into_main()
...
def read_csv_data(path_to_csv, spark):
    try:
        ...
        df = df.withColumn('code', col('francdn'))
        return df
    except Exception as e:
...
def data_to_insert_staging_table(csv, hash_table_db, url, properties):
    try:
        ...
        csv_to_insert_stage.write.jdbc(url=url, table="franchise_insert_staging", mode="append", properties=properties)
        ...
        missing_code_to_hash_table.write.jdbc(url=url, table="franchise_hash", mode="append", properties=properties)
...
def data_to_update_staging_table(csv, hash_table_db, url, properties):
    try:
        ...
        csv_to_update_stage.write.jdbc(url=url, table="franchise_update_staging", mode="append", properties=properties)    
        ...
        start_hash_time = time.time()
        mismatch_hash_to_hash_temp.write.jdbc(url=url, table="franchise_hash_staging", mode="append", properties=properties)    
        log_time(start_time=start_hash_time, str="csv hash to hash stage")
    except Exception as e:
...
def main(local_file_path, spark, properties, url):
    try:
       ...
       # Assuming 'csv' is your DataFrame
        ...
        hash_table_db = spark.read.jdbc(url=url, table="franchise_hash", properties=properties)
...
def franchise_table_step_2_insert_into_main(spark, properties, db_url):
    connection = None
    try:
        # psycopg2 Property
        logger.info("Step 2 Started . method Name franchise_table_step_2_insert_into_main")
        ...
        sourceDF = spark.read.jdbc(db_url, table="franchise_insert_staging", properties=properties)
        ...
        sourceDF.select(columns_to_insert).write.jdbc(db_url, table="franchise", mode="append", properties=properties)
        
        # Insert data count into the logging dictionary
        inbound_spark_management["data_insert_count"] = sourceDF.count()
        
        # Truncate staging table
        cursor.execute("TRUNCATE TABLE franchise_insert_staging")
        connection.commit()
        ...
        # Log success message
        inbound_spark_management["status"] = "Success"
        inbound_spark_management["success_message"] = "franchise_table_step_2_insert_into_main executed successfully"
    except Exception as e:
        logger.error(f"Error in franchise_table_step_2_insert_into_main: {e}")
...
def franchise_table_step_3_update_into_main():
    connection = None
    try:
        logger.info("----------------Step 3 Started Method Name franchise_table_step_3_update_into_main ")
        # psycopg2 Property
        connection = psycopg2.connect(user=db_user, password=db_password, host=db_host, port=db_port, database=db_name)
        cursor = connection.cursor()
        ...
        sql_update_query  = """
            ...
            SET
                ...
                frands = sourceTable.frands,
                frprotect = sourceTable.frprotect,
                frovr = sourceTable.frovr,
                froemflag = sourceTable.froemflag,
                frdstflag = sourceTable.frdstflag,
                freucflag = sourceTable.freucflag,
                frmerpbook = sourceTable.frmerpbook,
                freucpbook = sourceTable.freucpbook,
                fransmsg = sourceTable.fransmsg,
                paexcept = sourceTable.paexcept,
            ...
            FROM 
                franchise_update_staging AS sourceTable
            WHERE
                franchise.francdn = sourceTable.francdn;

            TRUNCATE TABLE franchise_update_staging;

            UPDATE franchise_hash  
            SET
                hash_value = temp.hash_value
            FROM
                franchise_hash_staging temp
            WHERE
                franchise_hash.code = temp.code;

            TRUNCATE TABLE franchise_hash_staging;
        ...
        logger.error(f"Error in franchise_table_step_3_update_into_main: {e}")
        inbound_spark_management["status"] = "Failure"
        inbound_spark_management["error_message"] = f'Error in franchise_table_step_3_update_into_main: {e}'
    finally:
        if connection:
            cursor.close()
            connection.close()
